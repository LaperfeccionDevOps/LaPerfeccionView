...
const PanelGerencialRRLLView = () => {
  ...
  const consultarGestionMensual = async (periodo) => {
    ...
    try {
      const url =
        `${obtenerBaseApi()}` +
        `/gestion-mensual-indicadores/RRLL_RETIROS/PANEL_GERENCIAL` +
      ...
      setAnalisisMes(resultado?.gestionMensual?.analisisMes || '');
  ...
  const guardarAnalisisAlSalir = async () => {
    ...
    try {
      ...
      const url =
        `${obtenerBaseApi()}` +
        `/gestion-mensual-indicadores/RRLL_RETIROS/PANEL_GERENCIAL` +
        `/${periodoMensualSeleccionado.anio}/${periodoMensualSeleccionado.mes}/gestion`;
      ...
      console.error(
        'Error guardando análisis mensual del Panel Gerencial RRLL:',
        error
      );
      setErrorGestion(
        error instanceof Error
  ...
  const guardarPlanAccion = async () => {
    ...
    try {
      ...
      console.error(
        'Error guardando plan de acción del Panel Gerencial RRLL:',
        error
      );
      setErrorGestion(
        error instanceof Error
  ...
  const guardarGestionMensual = async () => {
    ...
    if (permisos.puedeEditarAnalisis && analisisMes.trim()) {
      body.analisisMes = analisisMes.trim();
    }
    ...
    if (
      ...
      body.actividadesPlanAccion = actividadesPlanAccion.map((item) => ({
        idActividad: item.idActividad || undefined,
        actividad: String(item.actividad || '').trim(),
        fechaCompromiso: item.fechaCompromiso,
      }));
  ...
  const guardarCalificacionesActividades = async () => {
    ...
    try {
      ...
      const url =
        `${obtenerBaseApi()}` +
        `/gestion-mensual-indicadores/RRLL_RETIROS/PANEL_GERENCIAL` +
        `/${periodoMensualSeleccionado.anio}/${periodoMensualSeleccionado.mes}/actividades/calificaciones`;
      ...
      setMensajeGestion(
        'Calificaciones guardadas correctamente. El plan de acción quedó cerrado para RRLL.'
      );
    } catch (error) {
      console.error(
        'Error guardando calificaciones del Panel Gerencial RRLL:',
        error
      );
      setErrorGestion(
        error instanceof Error
...
const GestionMensualRRLL = ({
  ...
  const formatearFechaHoraGestionRRLL = (fecha) => {
    if (!fecha) {
      return '—';
    }
            ...
            <p className="mt-3 text-xs text-gray-500">
              {detalle.usuarioAnalisis ? (
                <>
                  ...
                  {' · '}
                  {formatearFechaHoraGestionRRLL(detalle.fechaAnalisis)}
                </>
              ) : (
            ...
            <p className="mt-3 text-xs text-gray-500">
              {detalle.usuarioPlanAccion ? (
                <>
                  ...
                  {' · '}
                  {formatearFechaHoraGestionRRLL(detalle.fechaPlanAccion)}
                </>
              ) : (
                'Pendiente de diligenciar.'
              )}
            ...
            <div className="flex-1">
              ...
              {detalle.usuarioCalificacion && actividadesCalificadas > 0 && (
                <p className="mt-2 text-xs text-gray-500">
                  ...
                  {' · '}
                  {formatearFechaHoraGestionRRLL(detalle.fechaCalificacion)}
                </p>
              )}
            </div>
