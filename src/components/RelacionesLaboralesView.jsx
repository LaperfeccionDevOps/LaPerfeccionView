...
export default function RelacionesLaboralesView() {
  ...
  const [loadingSolicitudesViernes, setLoadingSolicitudesViernes] = useState(false);
  const [errorSolicitudesViernes, setErrorSolicitudesViernes] = useState("");
...
const [mensajeEntrevista, setMensajeEntrevista] = useState({
 ...
 const handleDescargarExcel = async () => {
  ...
  const url = `${API_BASE}/rrll-excel/exportar-retiros?fecha_inicio=${fechaInicioExcel}&fecha_fin=${fechaFinExcel}`;
...
const cargarSolicitudesViernes = async () => {
  try {
    ...
    const solicitudesBase = Array.isArray(data) ? data : [];
      ...
      solicitudesBase.map(async (solicitud) => {
        ...
        if (nombreBackend || numeroBackend) {
          return {
            ...
            NumeroDocumentoTrabajador: numeroBackend,
            TipoDocumentoTrabajador: tipoBackend || "CC",
          };
        }

        try {
          const idProceso = solicitud?.IdProcesoDisciplinario;
          ...
          const expediente = await respuestaExpediente.json();
          const trabajador =
          ...
          const nombreCompleto = String(
            trabajador?.NombreCompleto ||
              ...
              `${trabajador?.Nombres || expediente?.Nombres || ""} ${
                trabajador?.Apellidos || expediente?.Apellidos || ""
              }`
          ).trim();
          ...
          return {
            ...
            NombreCompletoTrabajador:
              nombreCompleto ||
              `Registro #${solicitud.IdRegistroPersonal}`,
            NumeroDocumentoTrabajador: numeroDocumento,
            TipoDocumentoTrabajador: tipoDocumento || "CC",
          };
        } catch (error) {
          console.error(
    ...
    setSolicitudesViernes(solicitudesEnriquecidas);
  } catch (error) {
    console.error("Error consultando solicitudes de viernes:", error);
    setSolicitudesViernes([]);
    setErrorSolicitudesViernes(
      error?.message || "No fue posible consultar las solicitudes pendientes."
    );
  } finally {
...
useEffect(() => {
  if (step === "solicitudes_viernes") {
    cargarSolicitudesViernes();
  }
}, [step]);

const abrirDecisionSolicitud = (solicitud, accion) => {
...
const resolverSolicitudViernes = async () => {
  if (!solicitudSeleccionada?.IdSolicitudAutorizacion) return;

  if (accionSolicitud === "aprobar" && !horarioSolicitud) {
    alert("Debe seleccionar el horario que se habilitará.");
    return;
  }
  ...
  try {
    ...
    const idSolicitud = solicitudSeleccionada.IdSolicitudAutorizacion;
    const endpoint =
    ...
    if (accionSolicitud === "aprobar") {
      const [horaInicio, horaFin] = horarioSolicitud.split("|");

      body = {
        HoraInicio: horaInicio,
        HoraFin: horaFin,
        UsuarioResuelve: "rrll",
        ObservacionResolucion: observacionSolicitud.trim() || null,
      };
    } else {
      body = {
        UsuarioResuelve: "rrll",
        ObservacionResolucion: observacionSolicitud.trim(),
      };
    }
    ...
    console.error("Error resolviendo solicitud de viernes:", error);
    alert(error?.message || "No fue posible resolver la solicitud.");
  } finally {
    ...
    <div className="p-4 md:p-6">
      <div className="bg-white rounded-2xl shadow-xl p-5 md:p-8 border-t-4 border-amber-500">
        ...
        <div className="mt-6 flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 md:flex-row md:items-center md:justify-between">
          ...
          >
            {loadingSolicitudesViernes ? "Actualizando..." : "Actualizar"}
          </Button>
        </div>
        ...
        {loadingSolicitudesViernes ? (
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2">
            {solicitudesViernes.map((solicitud) => (
              <article
              ...
              >
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                      Solicitud #{solicitud.IdSolicitudAutorizacion}
                    </p>
                    <h3 className="mt-1 text-lg font-bold text-gray-900">
                      Expediente disciplinario #{solicitud.IdProcesoDisciplinario}
                    </h3>
                  </div>
                ...
                <dl className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <dd className="mt-1 font-semibold text-gray-800">
                      ...
                      <span className="mt-1 block text-sm font-medium text-gray-600">
                        {solicitud.NumeroDocumentoTrabajador
                          ? `${solicitud.TipoDocumentoTrabajador || "CC"} ${
                              solicitud.NumeroDocumentoTrabajador
                            }`
                          : `Registro #${solicitud.IdRegistroPersonal}`}
                      </span>
                  ...
                  <div className="rounded-xl bg-gray-50 p-3">
                    ...
                    <dd className="mt-1 font-semibold text-gray-800">
                      {formatearFechaSolicitud(solicitud.FechaSolicitada)}
                    </dd>
                  </div>
                  ...
                  <div className="rounded-xl bg-gray-50 p-3 sm:col-span-2">
                    ...
                    <dd className="mt-1 font-semibold text-gray-800">
                      {solicitud.UsuarioSolicita || "Operaciones"}
                    </dd>
                  </div>
                ...
                <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => abrirDecisionSolicitud(solicitud, "rechazar")}
                  ...
                  <Button
                    type="button"
                    onClick={() => abrirDecisionSolicitud(solicitud, "aprobar")}
          ...
          <div className="w-full max-w-xl rounded-2xl bg-white p-5 shadow-2xl md:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Expediente #{solicitudSeleccionada.IdProcesoDisciplinario} · Viernes{" "}
                  {formatearFechaSolicitud(solicitudSeleccionada.FechaSolicitada)}
                </p>
              </div>
            ...
            {accionSolicitud === "aprobar" ? (
              <div className="mt-5">
                ...
                <select
                  ...
                  onChange={(event) => setHorarioSolicitud(event.target.value)}
                  className="mt-2 h-12 w-full rounded-md border border-gray-300 bg-white px-3 text-sm"
                >
                  <option value="">Seleccione un horario</option>
                  {HORARIOS_SOLICITUD_VIERNES.map((horario) => (
            ...
            <div className="mt-5">
              ...
              <textarea
                ...
                onChange={(event) => setObservacionSolicitud(event.target.value)}
                rows={4}
                placeholder={
                  accionSolicitud === "aprobar"
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <Button
                type="button"
                onClick={resolverSolicitudViernes}
                disabled={procesandoSolicitud}
                className={
                  accionSolicitud === "aprobar"
